...
public class Http2OrHttpHandler extends ApplicationProtocolNegotiationHandler {
	...
	private static void configureHttp2(ChannelHandlerContext ctx) {
		...
		InboundHttp2ToHttpAdapter listener = new InboundHttp2ToHttpAdapterBuilder(connection).propagateSettings(true).validateHttpHeaders(false)
				.maxContentLength(MAX_CONTENT_LENGTH).build();

		ctx.pipeline()
				.addLast(new HttpToHttp2ConnectionHandlerBuilder().frameListener(listener)
